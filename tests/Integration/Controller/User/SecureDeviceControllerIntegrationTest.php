...
final class SecureDeviceControllerIntegrationTest extends WebTestCase
{
    ...
    public function testSecureDeviceQrIdentityReturnsGeneratedResponse(): void
    {
        ...
        $requestMapper = $this->createMock(SecureDeviceQrIdentityRequestMapper::class);
        $requestMapper
            ->expects(self::once())
            ->method('map')
        ...
        static::getContainer()->set(SecureDeviceQrIdentityRequestMapper::class, $requestMapper);
        ...
        $client->request(
            'POST',
            '/api/user/secure-device/qr-identity',
            server: [
    ...
    public function testSecureDeviceQrIdentityReturnsStandardErrorPayloadOnValidationFailure(): void
    {
        $client = static::createClient();
        ...
        $payloadValidator
            ->expects(self::once())
            ->method('getValidatedPayload')
            ->willThrowException(new \InvalidArgumentException('Invalid secure device registration payload.'));
        static::getContainer()->set(PayloadValidator::class, $payloadValidator);
